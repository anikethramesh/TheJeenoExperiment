...
class LLMArbitrator(ArbitratorBackend):
    ...
    def arbitrate(
        ...
        scene_api = (
            ...
            "  scene.agent_x, scene.agent_y  — agent position (float; integral on a 2D grid)\n"
            "  scene.agent_coord             — (x, y) or (x, y, z) tuple, N-dimensional\n"
            ...
            "  scene.manhattan_distance_from_agent(obj) → float\n"
            "  SceneObject: .object_type, .color, .x, .y, optional .z, and .coord tuple\n"
            "  math and geometry are pre-injected; prefer geometry.manhattan/geometry.euclidean\n"
            "    over coord tuples (e.g. geometry.euclidean(obj.coord, scene.agent_coord)) — they\n"
            "    are N-dimensional and float-capable, so they work on 2D and 3D substrates.\n"
